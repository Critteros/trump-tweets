...
def waterfall_legacy(
    ...
    if shap.utils.safe_isinstance(features, "pandas.core.series.Series"):
    ...
    for i in range(num_individual):
        ...
        if features is None:
            ...
            yticklabels[rng[i]] = (
                shap.utils.format_value(features[order[i]], "%0.03f")
                + " = "
    ...
    for i in range(len(pos_inds)):
        ...
        if pos_low is not None and i < len(pos_low):
            plt.errorbar(
                ...
                ecolor=shap.plots.colors.light_red_rgb,
            )

        txt_obj = plt.text(
            pos_lefts[i] + 0.5 * dist,
            pos_inds[i],
            shap.utils.format_value(pos_widths[i], "%+0.02f"),
    ...
    for i in range(len(neg_inds)):
        ...
        if neg_low is not None and i < len(neg_low):
            plt.errorbar(
                ...
                ecolor=shap.plots.colors.light_blue_rgb,
            )

        txt_obj = plt.text(
            neg_lefts[i] + 0.5 * dist,
            neg_inds[i],
            shap.utils.format_value(neg_widths[i], "%+0.02f"),
            horizontalalignment="center",
            verticalalignment="center",
            color="white",
            fontsize=12,
    ...
    ax2.set_xticklabels(
        [
            "\n$E[f(X)]$",
            "\n$ = " + shap.utils.format_value(expected_value, "%0.03f") + "$",
        ],
        fontsize=12,
        ha="left",
    )
    ...
    ax3.set_xticklabels(
        ["$f(x)$", "$ = " + shap.utils.format_value(fx, "%0.03f") + "$"],
        fontsize=12,
        ha="left",
    )
